...
#     # Handle cross-layer transcoder vs single layer SAE
#     if "hook_points" in sae.config:
#         # Cross-layer transcoder: get activations for all layers and target
#         hook_points = sae.config["hook_points"] + [sae.config["target_hook_point"]]
#         with torch.no_grad():
#             _, cache = model.run_with_cache(
#                 batch_tokens,
#                 names_filter=hook_points,
#             )
#         # Get input layers and target
#         input_acts = torch.cat([cache[hook_point].unsqueeze(0) for hook_point in sae.config["hook_points"]], dim=0)
#         input_acts = input_acts.permute(1, 2, 0, 3).reshape(-1, len(sae.config["hook_points"]), sae.config["act_size"])
#         target_acts = cache[sae.config["target_hook_point"]].reshape(-1, sae.config["act_size"])

#         # Get SAE output
#         sae_output = sae((input_acts, target_acts))["sae_out"].reshape(batch_tokens.shape[0], batch_tokens.shape[1], -1)
#         hook_point = sae.config["target_hook_point"]  # Use target hook point for evaluation
#     else:
#         # Single layer SAE
...
#         batch_tokens,
#         fwd_hooks=[(hook_point, partial(reconstr_hook, sae_out=sae_output))],
#         return_type="loss",
#     ).item()
...
#         batch_tokens,
#         fwd_hooks=[(hook_point, mean_abl_hook)],
#         return_type="loss",
#     ).item()
